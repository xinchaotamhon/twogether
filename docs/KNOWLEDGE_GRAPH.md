...
Actual nodes are grouped by their existing kind into bộ kiến thức, thân nguyên lý, cành cơ chế, and lá chuyển giao, then placed in a centered SVG-backed graph stage. Solid links show part_of structure; dashed links show prerequisite edges. Hovering, focusing, or tapping a node selects it and reveals its purpose, aggregate descendant card count, stability summary, and prerequisite labels. The virtual root aggregates the whole collection and is a navigation aid, not a published learning card or an ontological claim.
