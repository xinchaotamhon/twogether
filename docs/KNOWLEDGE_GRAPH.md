...
As of ADR-0021, the renderer chooses one deterministic incoming relation per node to create a readable spanning-tree skeleton: prefer the deepest legitimate source and use `part_of` to break equal-depth ties. Parents are centered over descendant leaves, so one bottom root visibly forks upward instead of placing every module in one horizontal strip. This is only a presentation projection; all other legitimate prerequisite/part-of relations stay in the graph, dashed/solid overlays and accessible table. The pending vocabulary-review node belongs under the lexicon branch and becomes studyable only after at least one vocabulary card is Human-approved.
