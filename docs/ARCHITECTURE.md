...
- Supabase Auth + Postgres with Row Level Security (RLS) for anonymous device pairing, shared content, and per-learner state. It is selected by ADR-0007 and implemented locally, but the remote schema is not yet provisioned or security-tested.
- The repository includes a Supabase browser adapter, additive migrations, server-derived streak and safe local-history import. Cloud authority still requires owner-approved migration/seed/bootstrap plus real two-learner RLS/conflict gates; local storage remains the reversible fallback.
...
Per-learner tables: `learner_card_states`, `review_events`, `study_sessions`, `daily_qualifications`, derived streak projection, `learner_preferences`, `private_notes`. The migrations implement this event-derived design, but it is not authoritative until applied and verified remotely.
...
The safe fallback is local-first: the shell, content, reviews, runs, and streak survive reloads in the same browser but do not follow the learner elsewhere. Cloud mode uses anonymous device pairing, stable idempotency keys, a retained local import backup, and server-authoritative accepted review/qualification events. It still awaits remote provisioning and real RLS/conflict verification. Full offline multi-device merge is a later gate, not an implied feature of “PWA”.
