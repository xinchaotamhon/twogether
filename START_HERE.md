...
Read `docs/CONTENT_AUTHORING.md`, `docs/CONTENT_STYLE_HIEP_HOANG.md` and `docs/CURRICULUM_EXPANSION_ROADMAP.md`. Immutable source/review packets live under `content/drafts/`; exact Human approvals live under `content/reviews/`; `src/approvedCurriculum.ts` derives published runtime without mutating either. Consult `40-State/CURRENT_STATE.md` for the currently active approval rather than recording a changing version here. Place every new card at one or more legitimate DAG prerequisites with model answer, misconception, matching transfer prompt/answer, provenance, and Human-review status; then run focused content/support checks plus all cumulative gates. AI generation, fresh-agent critique and in-app visibility are never publication.
