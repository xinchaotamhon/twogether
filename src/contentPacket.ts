...
export interface PacketValidation { valid: boolean; errors: string[] }

const CARD_TYPES: readonly CardType[] = ["core_recall", "mechanism", "contrast", "boundary", "application", "production"];
const isRecord = (value: unknown): value is Record<string, unknown> => !!value && typeof value === "object";
const isString = (value: unknown): value is string => typeof value === "string" && value.trim().length > 0;

export function validateCardPacket(value: unknown): PacketValidation {
  const errors: string[] = [];
  if (!isRecord(value)) return { valid: false, errors: ["packet phải là object"] };
  if (value.schema_version !== CARD_PACKET_SCHEMA) errors.push("schema_version không được hỗ trợ");
  if (!isString(value.packet_id)) errors.push("thiếu packet_id");
  if (!isString(value.created_at) || Number.isNaN(Date.parse(value.created_at))) errors.push("created_at phải là ISO date");
  const actor = value.created_by;
  if (!isRecord(actor) || (actor.actor_type !== "human" && actor.actor_type !== "ai") || !isString(actor.actor_id)) errors.push("created_by không hợp lệ");
  const sources = value.sources;
  if (!Array.isArray(sources)) errors.push("sources phải là array");
  else sources.forEach((source, index) => {
    if (!isRecord(source) || !isString(source.source_ref) || source.raw_private_content_included !== false) errors.push(`sources[${index}] thiếu provenance an toàn`);
  });
  const cards = value.cards;
  if (!Array.isArray(cards) || cards.length === 0) errors.push("cards phải là array không rỗng");
  else {
    const ids = new Set<string>();
    cards.forEach((card, index) => {
      if (!isRecord(card)) { errors.push(`cards[${index}] không phải object`); return; }
      if (!isString(card.id) || ids.has(card.id)) errors.push(`cards[${index}].id thiếu hoặc trùng`);
